...
class FlashcardEditor(tk.Toplevel):
    ...
    def __init__(self, master: tk.Misc | None, json_path: str) -> None:
        ...
        self.title("Flashcard Editor — " + os.path.basename(json_path))
        ...
        self.listbox = tk.Listbox(left, width=45, height=30, activestyle="none", fg="white", bg="#202324", bd=0, highlightthickness=1, highlightbackground="#444")
        ...
        save_btn = tk.Button(right, text="💾 Speichern", font=("SF Pro Display", 14, "bold"), command=self._save_changes)
    ...
    def _ai_refine(self):
        prompt = simpledialog.askstring("AI‑Überarbeiten", "Beschreibe, was geändert werden soll.\n(Beispiel: ‚Formuliere die Fragen kürzer‘)")
        if not prompt:
            return
        ...
        if messagebox.askyesno("AI‑Ergebnis", f"{len(new_cards)} Karten vom Modell erhalten. Änderungen übernehmen?"):
            ...
            self._save_changes()  # writes & repopulates listbox
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python flashcard_editor.py path/to/batch.json") # python3 flashcard_editor.py flashcards/die_Grundzüge_der_medizinische_3_4.json
        sys.exit(1)
